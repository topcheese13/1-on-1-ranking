...
export default class GameLink extends React.Component {
    render() {
        return (
            <Link to={`/game/${this.props.gameID}`} className="listItem-link listItems-match">
                <span className="listItem-rank">{this.props.index}</span>
                <span className="listItem-winner">
                    {this.props.leader && <svg className="listItem-winnerIcon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 270 162.555">
                        <title>Crown</title>
                ...
                <span className="listItem-loser" title="Loser">{this.props.loser}</span>
            </Link>
        );
    }
};
